...
def stampBuildMenus():
    ...
    if not Stamps_MenusLoaded:
        ...
        m = nuke.menu('Nuke').addMenu('Stamps', icon="stamps.png")
        m.addCommand('Make Stamp', 'stamps.goStamp()', STAMPS_SHORTCUT, icon="stamps.png")
        m.addCommand('Add tag\/s to selected nodes', 'stamps.addTags()')
        m.addCommand('Rename Stamp tag', 'stamps.renameTag()')
        m.addCommand('Refresh all Stamps', 'stamps.refreshStamps()')
        m.addCommand('Selected/Reconnect by Name', 'stamps.selectedReconnectByName()')
        m.addCommand('Selected/Reconnect by Title', 'stamps.selectedReconnectByTitle()')
        m.addCommand('Selected/Reconnect by Selection', 'stamps.selectedReconnectBySelection()')
        m.menu('Selected').addSeparator()
        m.addCommand('Selected/Refresh', 'stamps.refreshStamps(nuke.selectedNodes())')
        m.addCommand('Selected/Select Similar', 'stamps.selectedSelectSimilar()')
        m.addCommand('Selected/Toggle auto-rec... by title ', 'stamps.selectedToggleAutorec()')

        m.addCommand('Advanced/Convert all Stamps to NoOp', 'stamps.allToNoOp()')
        m.addSeparator()
        m.addCommand('GitHub', 'stamps.showInGithub()')
        m.addCommand('Nukepedia', 'stamps.showInNukepedia()')
        m.addSeparator()
        m.addCommand('Video tutorials', 'stamps.showVideo()')
        m.addCommand('Documentation (.pdf)', 'stamps.showHelp()')
