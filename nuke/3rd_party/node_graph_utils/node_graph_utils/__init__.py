...
def install_menus(icons_root=None, install_experimental_menus=False):
    ...
    organize_menu = nuke.menu('Nuke').addMenu('OrganizeNodes', icon=_get_icon('align_center_x'))

    organize_menu.addCommand('Align Nodes - Left', partial(align_selection, dag.LEFT), 'ctrl+alt+4', shortcutContext=2,
                             icon=_get_icon('align_left'))
    organize_menu.addCommand('Align Nodes - Right', partial(align_selection, dag.RIGHT), 'ctrl+alt+6', shortcutContext=2,
                             icon=_get_icon('align_right'))
    organize_menu.addCommand('Align Nodes - Center X', partial(align_selection, dag.CENTER_X), 'ctrl+alt+5',
                             shortcutContext=2, icon=_get_icon('align_center_x'))
    organize_menu.addCommand('Align Nodes - Top', partial(align_selection, dag.UP), 'ctrl+alt+8', shortcutContext=2,
                             icon=_get_icon('align_top'))
    organize_menu.addCommand('Align Nodes - Bottom', partial(align_selection, dag.DOWN), 'ctrl+alt+2', shortcutContext=2,
                             icon=_get_icon('align_bottom'))
    organize_menu.addCommand('Align Nodes - Center Y', partial(align_selection, dag.CENTER_Y), 'ctrl+alt+shift+5',
    ...
    organize_menu.addCommand('Distribute Nodes Horizontally', partial(interval, dag.AXIS_X), 'ctrl+alt+0',
                             shortcutContext=2, icon=_get_icon('space_x'))
    organize_menu.addCommand('Distribute Nodes Vertically', partial(interval, dag.AXIS_Y), 'ctrl+alt+shift+0',
                             shortcutContext=2, icon=_get_icon('space_y'))
    organize_menu.addCommand('Mirror Nodes', mirror_nodes, 'ctrl+alt+/', shortcutContext=2, icon=_get_icon('mirror_x'))
